...
def ricevi_senfinajxaj_vortoj():
    ...
    memvortaj_radikoj = list(filter(lambda rad: el_radikoj['tipo'][rad] == '.', tutaj_sb_radikoj))
    kompleksaj_radikoj = list(filter(lambda rad: el_radikoj['tipo'][rad] == ':', tutaj_sb_radikoj))
    senfinajxaj_vortoj = memvortaj_radikoj + kompleksaj_radikoj # Слова, у которых в словарном виде нет окончания
    return senfinajxaj_vortoj

def ricevi_sufiksoj():
    sufiksoj = list(filter(lambda rad: sb_sufiksoj['nivelo'][rad] == 3, sb_sufiksoj['signifo'].keys()))
    return sufiksoj
